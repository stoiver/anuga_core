...
# Compute mode follows the domain default (ANUGA_DEFAULT_COMPUTE_MODE, serial);
# pass -mpm 1|2 on the command line to force legacy/unified.
...
# Honor the domain's env-based default compute mode (set at construction from
# ANUGA_DEFAULT_COMPUTE_MODE); an explicit -mpm on the command line overrides it.
_mpm = getattr(args, 'multiprocessor_mode', None)
if _mpm is not None:
    domain.set_multiprocessor_mode(_mpm)
