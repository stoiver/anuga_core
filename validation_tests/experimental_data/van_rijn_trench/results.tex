...
(\texttt{set\_deposition(} \texttt{adaptation='armanini')}) over-corrects this, so the
...
The case as committed runs with the instantaneous exchange, so that what it
validates is the default closure. The options below were each run on test 3
...
\begin{tabular}{L{4.0cm}T{3.6cm}L{1.1cm}L{1.6cm}rr}
\hline
\textrm{option} & \textrm{chosen by} & RMS (cm) & deepest (m) & slope & toe \\
\hline
measured & & & 6.57, 0.079 & +0.17 & $-0.26$ \\
\hline
instantaneous exchange (the case) & adaptation='none' & 2.85 & 7.10, 0.039 & +0.20 & $-0.15$ \\
Galappatti lag [D-3] & adaptation='armanini' & 3.7 & & & \\
carried near-bed ratio [D-4] & adaptation='carried' & 2.7 & & & \\
two-layer, near-bed layer $a$ [D-5] & adaptation='two\_layer' & 2.66 & 5.50, 0.084 & +0.08 & $-0.20$ \\
...
two-layer $0.2h$, log-law velocity split [D-5v] & layer\_fraction=0.2, velocity\_profile=True & \textbf{1.74} & 6.22, 0.054 & +0.17 & $-0.19$ \\
...
instantaneous exchange the case runs with.}
\end{figure}
}{}
