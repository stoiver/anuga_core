...
holding the whole 24~cm channel at 15 cells across, 18 over the sand and
43 around the pier, on a coarser reservoir; it releases the
...
\end{center}
\caption{The mesh: the flume carries 15 cells across its 24~cm width from
the gate to the exit, 18 over the erodible sand and 43 around the pier,
which is cut out as a hole with reflective edges; the reservoir, where the
flow is a slow drawdown, takes the overall cap of 26~mm. The three
full-width reaches are delimited by breaklines and their resolution set by
a marker point inside each, rather than by a region polygon inset from the
walls, which would leave a sliver the mesher fills with a band of tiny
triangles. The reservoir and the flume are one
continuous mesh, so the mouth of the flume is interior: a breakline along
it puts cell edges on the gate line, which keeps the contraction and the
crest of the sill that \texttt{ZARAGOZA\_GATE\_TIME} raises from being
ragged. The step either side of the mouth is a pair of wall segments.}
\end{figure}
}{}
...
6~mm over the window after the first dam-break: \anuga{} gives 3.5 and
3.8~mm. What the case actually tests is the scour at the pier. Over a ring
2--4~cm from the pier centre the flume lowered the bed by 10.4~mm (P1) and
12.6~mm (P2) in the first event; with the pier-scour correction described
below the model gives 4.8 and 9.6, and without it 2.9 and 6.0. The deepest
points are 32 and 53~mm measured against 21 and 21 (the 53 exceeds the
sand thickness and is a Kinect artefact at the pier's edge), and the wake
deposit overshoots, +5.8 and +7.2 against +3.0 and +2.4, which is the cost
of the correction. Neither a 40$^\circ$ angle of repose, nor a mesh
...
on the centreline at 1.2~m and a trough at 1.4~m. Resolving the whole flume
at 1~cm, which the case now does, does not produce them. The model's reservoir drops from 80 to
