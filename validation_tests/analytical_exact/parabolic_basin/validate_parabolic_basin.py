...
class Test_results(unittest.TestCase):
    ...
    def test_parabolic_basin(self):
        if verbose:
            print()
            print(indent + 'Running simulation script')

        res = anuga.run_anuga_script('numerical_parabolic_basin.py', args=args)
        assert res == 0, 'numerical_parabolic_basin.py failed with return code %d' % res
        ...
        # Use all centroids — analytic_cannal depends only on x (quasi-1D problem)
        ...
        # Compute error only over wet cells to avoid divide-by-zero issues
        wet = numpy.abs(stage_ana) > 1e-6
        if numpy.any(wet):
