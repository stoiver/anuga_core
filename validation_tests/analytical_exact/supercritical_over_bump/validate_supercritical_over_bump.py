"""Validate supercritical flow over a bump against analytical solution."""
...
class Test_results(unittest.TestCase):
    ...
    def test_supercritical_over_bump(self):
        if verbose:
            print()
            print(indent + 'Running simulation script')

        res = anuga.run_anuga_script('numerical_supercritical.py', args=args)
        assert res == 0, 'numerical_supercritical.py failed with return code %d' % res

        if verbose:
            print(indent + 'Testing accuracy against analytical solution')

        import anuga.utilities.plot_utils as util
        import analytical_supercritical as analytic
