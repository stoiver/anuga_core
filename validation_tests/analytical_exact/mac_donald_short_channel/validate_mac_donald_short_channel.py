"""Validate MacDonald short channel flow against analytical solution."""
...
class Test_results(unittest.TestCase):
    ...
    def test_mac_donald_short_channel(self):
        if verbose:
            print()
            print(indent + 'Running simulation script')

        res = anuga.run_anuga_script('numerical_MacDonald.py', args=args)
        assert res == 0, 'numerical_MacDonald.py failed with return code %d' % res

        if verbose:
            print(indent + 'Testing accuracy against analytical solution')

        import anuga.utilities.plot_utils as util
        import analytical_MacDonald as analytic
