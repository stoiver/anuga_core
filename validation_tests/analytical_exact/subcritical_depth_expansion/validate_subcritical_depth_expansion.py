"""Validate subcritical flow with depth expansion against analytical solution."""
...
class Test_results(unittest.TestCase):
    ...
    def test_subcritical_depth_expansion(self):
        if verbose:
            print()
            print(indent + 'Running simulation script')

        res = anuga.run_anuga_script('numerical_depth_expansion.py', args=args)
        assert res == 0, 'numerical_depth_expansion.py failed with return code %d' % res

        if verbose:
            print(indent + 'Testing accuracy against analytical solution')

        import anuga.utilities.plot_utils as util
        import analytical_depth_expansion as analytic
        from numpy import arange

        p_st = util.get_output('numerical_depth_expansion.sww')
