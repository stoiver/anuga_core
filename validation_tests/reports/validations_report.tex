...
\author{Sudi Mungkasi, Stephen Roberts, Gareth Davies, Rudy van Drie}    %Type your name(s) here!
\maketitle
\tableofcontents
...
\anuga{} is \verb|http://anuga.anu.edu.au| and the source code is available in the github repository 
\verb|https://github.com/anuga-community/anuga_core|.



The method implemented in \anuga{} is a finite volume method used to
approximate the shallow water equations. Some mathematical explanation of the method
...
The results in this report were produced by \anuga{} version \majorR{}.\minorR{} at time \timeR.
The flow algorithm used was \alg{}, unless otherwise stated explicitly.

To get an automated report, you can run either run individual tests in each 
test directory \verb|python produce_results.py| or the complete report from the 
\verb|validation_tests/reports| directory (\verb|python validations_produce_results.py|). 
The complete report will run all the tests and produce a complete report. 
The individual test will only run the test in the corresponding directory and
 produce an individual report for that test.
...
\verb|\anuga_core\anuga\shallow_water\shallow_water_domain.py|.
...
%%======================
\chapter{Sediment transport and erosion} \label{ch:sediment}

The sediment cases are gathered here, analytical and experimental together,
because they exercise one set of closures: entrainment, deposition and its
adaptation, bedload, and the Exner coupling that moves the bed. The first
five have exact or reference solutions; the last four are laboratory
experiments, each with a diagram of the flume it reproduces.

\inputresults{../sediment/sediment_settling}
\inputresults{../sediment/sediment_erosion}
\inputresults{../sediment/sediment_settling_basin}
\inputresults{../sediment/sediment_equilibrium_flow}
\inputresults{../sediment/sediment_bed_hump}

\inputresults{../sediment/van_rijn_pickup_flume}
\inputresults{../sediment/van_rijn_trench}
\inputresults{../sediment/wang_ribberink_settling}
\inputresults{../sediment/zaragoza_pier_dambreak}
...
Within the \anuga{} script you can obtain command line parameters via
\begin{verbatim}
args = anuga.get_args()
alg = args.alg
