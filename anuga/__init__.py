...
try:
    ...
    # anuga/_version.py is generated by meson at build time and is gitignored,
    # so it is absent from an unbuilt source tree. Python searches the working
    # directory before site-packages, so this fires both when the tree has not
    # been built and when a perfectly good install is shadowed by running from
    # the repository root. The bare "No module named 'anuga._version'" points at
    # neither cause, so say what to do instead.
    if _exc.name != 'anuga._version':
        raise
    import os as _os
    raise ModuleNotFoundError(
        "anuga/_version.py is missing, so this copy of the anuga source tree "
        "has not been built.\n"
        "  Imported from: {}\n"
        "Either build it in place:\n"
        "    pip install --no-build-isolation -e .\n"
        "or, if anuga is already installed, run from a directory other than "
        "the repository root -- the working directory takes precedence over "
        "the installed package.".format(_os.path.dirname(__file__))
    ) from _exc
