"""
Simple water flow example using ANUGA  (DE_ader2 flow algorithm)

Water driven up a linear slope and time varying boundary,
similar to a beach environment.

Tests that the parallel DE_ader2 evolve produces the same gauge-station
...
class Test_parallel_sw_flow_de_ader2(unittest.TestCase):
    def test_parallel_sw_flow_de_ader2(self):
...
if __name__ == "__main__":
    if numprocs == 1:
        ...
        suite = unittest.TestLoader().loadTestsFromTestCase(
            Test_parallel_sw_flow_de_ader2)
        runner.run(suite)
    else:
        barrier()
        if myid == 0 and verbose:
