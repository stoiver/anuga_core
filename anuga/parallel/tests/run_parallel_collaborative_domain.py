...
from anuga.parallel import myid, numprocs, finalize
...
if numprocs > 1:
    if myid == 0 and verbose: print('DISTRIBUTING PARALLEL DOMAIN (collaborative)')
...
if numprocs > 1:
    if myid == 0:
        with open('collaborative_domain_parallel.txt', 'w') as fid:
    ...
    with open('collaborative_domain_sequential.txt', 'w') as fid:
        for i in range(len(l1list)):
            fid.write('%f %f %f %f %f %f %f %f %f\n' % (
                l1list[i][0], l1list[i][1], l1list[i][2],
                l2list[i][0], l2list[i][1], l2list[i][2],
