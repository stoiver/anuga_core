"""
Multi-rank GPU C-loop test for DE1 (RK2) flow algorithm.

Verifies that multiprocessor_mode=2 (C RK2 loop, C-level MPI ghost
exchange) produces results matching multiprocessor_mode=1 (Python RK2
loop, Python MPI ghost exchange) when run in parallel with DE1.

DE1 uses RK2 timestepping with second-order spatial reconstruction and
discontinuous elevation.  Both modes call the same core_kernels.c
functions for computation; the only difference is ghost exchange
(Python MPI vs C MPI).  After a correct build the results should be
bit-for-bit identical, so ATOL=1e-12 is used.  Ghost exchange errors
produce O(0.1-1.0) differences and are easily caught.
...
Run via pytest (auto-marked slow):
    ...
    pytest anuga/parallel/tests/test_parallel_sw_flow_gpu_de1.py

Or directly via MPI:

    mpiexec -np 4 python -m mpi4py anuga/parallel/tests/test_parallel_sw_flow_gpu_de1.py
"""
...
def run_simulation(mode, verbose=False):
    """Run a parallel DE1 simulation with the given multiprocessor_mode.

    Parameters
    ----------
    mode : int
        1 = Python RK2 + Python MPI ghost exchange
        2 = C RK2 + C-level MPI ghost exchange
    """
    ...
    domain.set_name(f'gpu_de1_mode{mode}')
...
class Test_parallel_sw_gpu_de1(unittest.TestCase):
...
if __name__ == '__main__':
    if numprocs == 1:
        ...
        if myid == 0:
            print(f'\n=== GPU halo exchange test DE1: {numprocs} ranks ===')
