        ...
        for root, dirs, files in os.walk(radar_dir):
            ...
            for filename in fnmatch.filter(files, pattern):
                ...
                if first:
                    ...
                    data_slice = data.variables[precip_name][:]/1000

                    data_accumulated = data_slice.copy()  # Put into new Accumulating ARRRAY

                    data_max_in_period = max(
                        np.max(data_slice), data_max_in_period)
                    ...
                    assert np.allclose(
                        self.time_step, new_time_step), "Timesteps not equal"
...
if __name__ == "__main__":
    ...
    if scenario == 'act':
        ...
        Catchment_file = join(
            BASE_DIR, 'ACT_Bdy/ACT_Entire_Catchment_Poly_Simple_UTM_55.csv')
        State_boundary_file = join(
            BASE_DIR, 'ACT_Bdy/ACT_State_Bdy_UTM_55.csv')
        Daily_plot_Vmax = 4
    ...
    p2 = [[689801.0, 6091501.0], [700200.0, 6091501.0], [
        700200.0, 6104600.0], [689801.0, 6104600.0], [689801.0, 6091501.0]]
    ...
    for tid in xrange(len(rain.times)):
        rain.plot_data(tid, plot_vmax=plot_vmax, save=False,
                       show=True, polygons=[p2, p3])
        if tid == 0:
            plt.colorbar()
        plt.pause(0.05)
        #ipdb.set_trace()
