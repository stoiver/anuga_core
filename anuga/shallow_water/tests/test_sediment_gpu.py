...
def test_deposition_agrees():
    def configure(mode):
        ...
        if mode != 1:
            d.set_multiprocessor_mode(mode)
            _require_mode_2(d)
        Sediment_transport_operator(d, name='sand', diameter=1.0e-4,
...
def test_entrainment_agrees():
    def configure(mode):
        ...
        Sediment_transport_operator(d, name='sand', diameter=1.0e-4, tau_c_star=0.04,
...
def test_the_rouse_near_bed_ratio_agrees():
    def configure(mode):
        ...
        if mode != 1:
            d.set_multiprocessor_mode(mode)
            _require_mode_2(d)
        Sediment_transport_operator(d, name='s', diameter=1e-4, initial_concentration=0.02)
        return d
...
def test_the_cohesive_erosion_route_agrees():
    def configure(mode):
        ...
        Sediment_transport_operator(d, name='silt', diameter=6.5e-5,
                             initial_concentration=0.0)
        return d
...
def test_exner_bed_evolution_agrees():
    def configure(mode):
        ...
        Sediment_transport_operator(d, name='sand', diameter=0.01, rho_s=2400.0,
