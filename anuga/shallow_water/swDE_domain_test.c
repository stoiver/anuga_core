...
// clang doesn't have openmp
   ...
   #include "omp.h"
#endif
#include "/home/samir/source/anuga_swde1_fast/anuga_core/anuga/shallow_water/sw_domain.h"
#include <omp.h> 
...
int __flux_function_central(double *q_left,double *q_right,double h_left,double h_right,double hle,double hre,double n1,double n2,double epsilon,double ze,double limiting_threshold,double g,double *edgeflux,double *max_speed,double *pressure_flux,double hc,double hc_n,long low_froude)
{
/*Compute fluxes between volumes for the shallow water wave equation
  ...
  */
  ...
  double denom;
  double inverse_denominator;
  double tmp;
  double local_fr;
  double v_right;
  double v_left;
  double q_left_rotated[3];
  double q_right_rotated[3];
  double flux_right[3];
  double flux_left[3];
...
//        (soundspeed_left*soundspeed_left + soundspeed_right*soundspeed_right + 1.0e-10))));
  if (low_froude == 1) {
    local_fr = sqrt((fmax(0.001,(fmin(1.0,(u_right * u_right + u_left * u_left + v_right * v_right + v_left * v_left) / (soundspeed_left * soundspeed_left + soundspeed_right * soundspeed_right + 1.0e-10))))));
  }
   else if (low_froude == 2) {
    local_fr = sqrt((u_right * u_right + u_left * u_left + v_right * v_right + v_left * v_left) / (soundspeed_left * soundspeed_left + soundspeed_right * soundspeed_right + 1.0e-10));
...
int _openmp__flux_function_central(double q_left0,double q_left1,double q_left2,double q_right0,double q_right1,double q_right2,double h_left,double h_right,double hle,double hre,double n1,double n2,double epsilon,double ze,double limiting_threshold,double g,double *edgeflux0,double *edgeflux1,double *edgeflux2,double *max_speed,double *pressure_flux,double hc,double hc_n,long low_froude)
{
  double edgeflux[3];
  double q_left[3];
  double q_right[3];
  ...
  q_right[1] = q_right1;
  q_right[2] = q_right2;
  ierr = __flux_function_central(q_left,q_right,h_left,h_right,hle,hre,n1,n2,epsilon,ze,limiting_threshold,g,edgeflux,max_speed,pressure_flux,hc,hc_n,low_froude);
...
double __adjust_edgeflux_with_weir(double *edgeflux,double h_left,double h_right,double g,double weir_height,double Qfactor,double s1,double s2,double h1,double h2,double *max_speed_local)
{
...
double _openmp__adjust_edgeflux_with_weir(double *edgeflux0,double *edgeflux1,double *edgeflux2,double h_left,double h_right,double g,double weir_height,double Qfactor,double s1,double s2,double h1,double h2,double *max_speed_local)
{
  double edgeflux[3];
  int ierr;
  edgeflux[0] =  *edgeflux0;
  edgeflux[1] =  *edgeflux1;
  edgeflux[2] =  *edgeflux2;
  ierr = (__adjust_edgeflux_with_weir(edgeflux0,h_left,h_right,g,weir_height,Qfactor,s1,s2,h1,h2,max_speed_local));
   *edgeflux0 = edgeflux[0];
   *edgeflux1 = edgeflux[1];
   *edgeflux2 = edgeflux[2];
  return ierr;
...
double _openmp_compute_fluxes_central(int K,int KI,int KI2,int KI3,int B,int RW,int RW5,int SubSteps,double D_edgelengths[KI],double D_normals[KI2],double D_edge_timestep[KI],double D_radii[K],double D_areas[K],long D_tri_full_flag[K],double D_stage_explicit_update[K],double D_xmom_explicit_update[K],double D_ymom_explicit_update[K],double D_stage_centroid_values[K],double D_height_centroid_values[K],double D_bed_centroid_values[K],double D_stage_edge_values[KI],double D_xmom_edge_values[KI],double D_ymom_edge_values[KI],double D_bed_edge_values[KI],double D_height_edge_values[KI],double D_edge_flux_work[KI3],double D_pressuregrad_work[KI],double D_max_speed[K],double D_stage_boundary_values[B],double D_xmom_boundary_values[B],double D_ymom_boundary_values[B],double D_boundary_flux_sum[SubSteps],long D_neighbours[KI],long D_neighbour_edges[KI],long D_edge_flux_type[KI],long D_edge_river_wall_counter[KI],long D_riverwall_rowIndex[RW],double D_riverwall_elevation[RW],double D_riverwall_hydraulic_properties[RW5],struct domain *D,double timestep)
...
// FIXME: limiting_threshold is not used for DE1
  ...
  double D_g = D -> g;
  double D_epsilon = D -> epsilon;
  long D_timestep_fluxcalls = D -> timestep_fluxcalls;
  long D_ncol_riverwall_hydraulic_properties = D -> ncol_riverwall_hydraulic_properties;
// Workspace (making them static actually made function slightly slower (Ole))
  ...
  double qr1;
  double qr2;
// Work array for summing up fluxes
  double edgeflux0;
  double edgeflux1;
  double edgeflux2;
...
// Index shorthands
  ...
  int ki3;
  int nm3;
  double local_timestep_array[8];
// Static local variable flagging already computed flux
  static long call = 0;
  static long timestep_fluxcalls = 1;
  static long base_call = 1;
// Flag 'id' of flux calculation for this timestep
  call++;
  if (D_timestep_fluxcalls != timestep_fluxcalls) {
    timestep_fluxcalls = D_timestep_fluxcalls;
    base_call = call;
  }
...
#pragma omp parallel for private (k)
  for (k = 0; k <= K - 1; k += 1) {
    D_stage_explicit_update[k] = 0.0;
    D_xmom_explicit_update[k] = 0.0;
    D_ymom_explicit_update[k] = 0.0;
  }
  for (k = 0; k <= K - 1; k += 1) {
    if (D_stage_explicit_update[k] > 0.0) {
      printf("D_stage_explicit_update[%d] = %e\n",k,D_stage_explicit_update[k]);
      printf("D_xmom_explicit_update[%d] = %e\n",k,D_xmom_explicit_update[k]);
      printf("D_ymom_explicit_update[%d] = %e\n",k,D_ymom_explicit_update[k]);
    }
  }
...
#pragma omp parallel for num_threads(8)
  for (int j = 0; j <= 7; j += 1) {
    local_timestep_array[j] = local_timestep;
  }
  double local_timestep_inner = local_timestep;
// For all triangles
//#pragma omp parallel
//  {
...
// double ql0, ql1, ql2;
// double qr0, qr1, qr2;
// double edgeflux0, edgeflux1, edgeflux2; // Work array for summing up fluxes
// double bedslope_work;
...
// double local_timestep_inner = local_timestep;
//int kmax = 0;
//int kmin = D->number_of_elements;
// printf("omp ID = %d, local_timestep %e,  substep_count %ld \n", ID, local_timestep, substep_count);
// printf("omp ID = %d, ncol riverwall_properties %ld \n", ID,  D->ncol_riverwall_hydraulic_properties);
// #pragma omp parallel for private(k, ki, ki2, ki3, n, m, max_speed_local, length, inv_area, zl, zr, \
                            h_left, h_right, \
                            z_half, ql0, ql1, ql2,  \
                            qr0, qr1, qr2, edgeflux0, edgeflux1, edgeflux2, bedslope_work, normal_x, normal_y, \
...
// Loop through neighbours and compute edge flux for each
    for (i = 0; i <= 2; i += 1) {
// Linear index to edge i of triangle k
      ki = 3 * k + i;
// k*6 + i*2
      ki2 = 2 * ki;
      ki3 = 3 * ki;
// Get left hand side values from triangle k, edge i
      ql0 = D_stage_edge_values[ki];
      ql1 = D_xmom_edge_values[ki];
      ql2 = D_ymom_edge_values[ki];
      zl = D_bed_edge_values[ki];
      hle = D_height_edge_values[ki];
      hc = D_height_centroid_values[k];
      zc = D_bed_centroid_values[k];
...
// or from boundary array (Quantities at neighbour on nearest face).
      n = D_neighbours[ki];
      hc_n = hc;
      zc_n = D_bed_centroid_values[k];
      if (n < 0) {
// Neighbour is a boundary condition
// Convert negative flag to boundary index
        m = -n - 1;
        qr0 = D_stage_boundary_values[m];
        qr1 = D_xmom_boundary_values[m];
        qr2 = D_ymom_boundary_values[m];
// Extend bed elevation to boundary
        zr = zl;
// hle;
        hre = fmax(qr0 - zr,0.0);
      }
...
// Neighbour is a real triangle
        hc_n = D_height_centroid_values[n];
        zc_n = D_bed_centroid_values[n];
        m = D_neighbour_edges[ki];
// Linear index (triangle n, edge m)
        nm = n * 3 + m;
        nm3 = nm * 3;
        qr0 = D_stage_edge_values[nm];
        qr1 = D_xmom_edge_values[nm];
        qr2 = D_ymom_edge_values[nm];
        zr = D_bed_edge_values[nm];
        hre = D_height_edge_values[nm];
      }
// Audusse magic
      z_half = fmax(zl,zr);
//// Account for riverwalls
      if (D_edge_flux_type[ki] == 1) {
        RiverWall_count = D_edge_river_wall_counter[ki];
// Set central bed to riverwall elevation
        z_half = fmax(D_riverwall_elevation[RiverWall_count - 1],z_half);
      }
// Define h left/right for Audusse flux method
      h_left = fmax(hle + zl - z_half,0.);
      h_right = fmax(hre + zr - z_half,0.);
      normal_x = D_normals[ki2];
      normal_y = D_normals[ki2 + 1];
// Edge flux computation (triangle k, edge i)
      _openmp__flux_function_central(ql0,ql1,ql2,qr0,qr1,qr2,h_left,h_right,hle,hre,normal_x,normal_y,D_epsilon,z_half,limiting_threshold,D_g,&edgeflux0,&edgeflux1,&edgeflux2,&max_speed_local,&pressure_flux,hc,hc_n,low_froude);
// Force weir discharge to match weir theory
      if (D_edge_flux_type[ki] == 1) {
        RiverWall_count = D_edge_river_wall_counter[ki];
        printf("RiverWall_count %ld\n",RiverWall_count);
        ii = (D_riverwall_rowIndex[RiverWall_count - 1] * D_ncol_riverwall_hydraulic_properties);
...
// Get h2, tailwater head / weir height at which we entirely use the shallow water solution
        Qfactor = D_riverwall_hydraulic_properties[ii];
        s1 = D_riverwall_hydraulic_properties[ii + 1];
        s2 = D_riverwall_hydraulic_properties[ii + 2];
        h1 = D_riverwall_hydraulic_properties[ii + 3];
        h2 = D_riverwall_hydraulic_properties[ii + 4];
// Reference weir height
        weir_height = fmax(D_riverwall_elevation[RiverWall_count - 1] - fmin(zl,zr),0.);
// Use first-order h's for weir -- as the 'upstream/downstream' heads are
//  measured away from the weir itself
        h_left_tmp = fmax(D_stage_centroid_values[k] - z_half,0.);
        if (n >= 0) {
          h_right_tmp = fmax(D_stage_centroid_values[n] - z_half,0.);
        }
         else {
...
// fluxes in such situations)
        if (D_riverwall_elevation[RiverWall_count - 1] > fmax(zc,zc_n)) {
// Weir flux adjustment
          _openmp__adjust_edgeflux_with_weir(&edgeflux0,&edgeflux1,&edgeflux2,h_left_tmp,h_right_tmp,D_g,weir_height,Qfactor,s1,s2,h1,h2,&max_speed_local);
        }
      }
// Multiply edgeflux by edgelength
      length = D_edgelengths[ki];
      edgeflux0 *= length;
      edgeflux1 *= length;
      edgeflux2 *= length;
      D_edge_flux_work[ki3 + 0] = -edgeflux0;
      D_edge_flux_work[ki3 + 1] = -edgeflux1;
      D_edge_flux_work[ki3 + 2] = -edgeflux2;
// bedslope_work contains all gravity related terms
      bedslope_work = length * (-D_g * 0.5 * (h_left * h_left - hle * hle - (hle + hc) * (zl - zc)) + pressure_flux);
      D_pressuregrad_work[ki] = bedslope_work;
...
// Compute the 'edge-timesteps' (useful for setting flux_update_frequency)
        tmp = 1.0 / fmax(max_speed_local,D_epsilon);
        D_edge_timestep[ki] = D_radii[k] * tmp;
// Update the timestep
        if (D_tri_full_flag[k] == 1) {
          if (max_speed_local > D_epsilon) {
// Apply CFL condition for triangles joining this edge (triangle k and triangle n)
// CFL for triangle k
            local_timestep_inner = fmin(local_timestep_inner,D_edge_timestep[ki]);
            if (D_edge_timestep[ki] < 0.0158) {
              printf(" %d %d, %d local_timestep_inner %e  %e\n",ID,k,i,local_timestep_inner,D_edge_timestep[ki]);
            }
            speed_max_last = fmax(speed_max_last,max_speed_local);
// printf(" %d %d, %d local_timestep_inner %f  %f\n", ID, k, i, local_timestep_inner, D->edge_timestep[ki]);
// if (n >= 0) {
//     // Apply CFL condition for neigbour n (which is on the ith edge of triangle k)
//    local_timestep_inner = fmin(local_timestep_inner, D->edge_timestep[nm]);
// }
          }
        }
      }
...
// Now add up stage, xmom, ymom explicit updates
  for (k = 0; k <= K - 1; k += 1) {
    double hc = fmax(D_stage_centroid_values[k] - D_bed_centroid_values[k],0.);
    for (i = 0; i <= 2; i += 1) {
// FIXME: Make use of neighbours to efficiently set things
      ...
      ki3 = ki * 3;
      n = D_neighbours[ki];
      D_stage_explicit_update[k] += D_edge_flux_work[ki3 + 0];
      D_xmom_explicit_update[k] += D_edge_flux_work[ki3 + 1];
      D_ymom_explicit_update[k] += D_edge_flux_work[ki3 + 2];
...
// boundary_flux_integral
      if (n < 0 & D_tri_full_flag[k] == 1 | (n >= 0 && D_tri_full_flag[k] == 1 & D_tri_full_flag[n] == 0)) {
// boundary_flux_sum is an array with length = timestep_fluxcalls
// For each sub-step, we put the boundary flux sum in.
        D_boundary_flux_sum[substep_count] += D_edge_flux_work[ki3];
      }
      D_xmom_explicit_update[k] -= D_normals[ki2] * D_pressuregrad_work[ki];
      D_ymom_explicit_update[k] -= D_normals[ki2 + 1] * D_pressuregrad_work[ki];
// end edge i
    }
// Normalise triangle k by area and store for when all conserved
// quantities get updated
    double inv_area = 1.0 / D_areas[k];
    D_stage_explicit_update[k] *= inv_area;
    D_xmom_explicit_update[k] *= inv_area;
    D_ymom_explicit_update[k] *= inv_area;
// end cell k
  }
...
double _xxx_compute_fluxes_central_parallel_data_flow(struct domain *D,double timestep)
{
// Local variables
  double max_speed_local;
  double length;
...
#pragma omp parallel for private (k)
  for (k = 0; ((long )k) <= D -> number_of_elements - 1; k += 1) {
...
// Define h left/right for Audusse flux method
      ...
      h_right = fmax(hre + zr - z_half,0.);
// Edge flux computation (triangle k, edge i)
      __flux_function_central(ql,qr,h_left,h_right,hle,hre,D -> normals[ki2],D -> normals[ki2 + 1],D -> epsilon,z_half,limiting_threshold,D -> g,edgeflux,&max_speed_local,&pressure_flux,hc,hc_n,low_froude);
// Force weir discharge to match weir theory
      if (D -> edge_flux_type[ki] == 1) {
...
// Reference weir height
        weir_height = fmax(D -> riverwall_elevation[RiverWall_count - 1] - fmin(zl,zr),0.);
// Use first-order h's for weir -- as the 'upstream/downstream' heads are
//  measured away from the weir itself
...
// Weir flux adjustment
          __adjust_edgeflux_with_weir(edgeflux,h_left_tmp,h_right_tmp,D -> g,weir_height,Qfactor,s1,s2,h1,h2,&max_speed_local);
        }
      }
// Multiply edgeflux by edgelength
...
double _xxx_compute_fluxes_central_original(struct domain *D,double timestep)
{
// Local variables
  double max_speed_local;
  double length;
...
#pragma omp parallel for private (hc,bmin,k) reduction (+:mass_error) firstprivate (minimum_allowed_height)
  for (k = 0; ((long )k) <= D -> number_of_elements - 1; k += 1) {
    hc = wc[k] - zc[k];
...
//min(bmin, wc[k]); //zv[3*k]-minimum_allowed_height);
          wv[3 * k] = bmin;
//min(bmin, wc[k]); //zv[3*k+1]-minimum_allowed_height);
          wv[3 * k + 1] = bmin;
//min(bmin, wc[k]); //zv[3*k+2]-minimum_allowed_height);
          wv[3 * k + 2] = bmin;
        }
      }
    }
...
int __find_qmin_and_qmax(double dq0,double dq1,double dq2,double *qmin,double *qmax)
{
...
// limited
  ...
  double r0 = 1.0;
  double phi = 1.0;
// to avoid machine accuracy problems.
...
#pragma omp parallel for private (r,r0,i) num_threads(3)
  for (i = 0; i <= 2; i += 1) {
    if (dqv[i] < -TINY) 
      r0 = qmin / dqv[i];
    if (dqv[i] > TINY) 
...
// wet-dry regions
// Highest depth ratio with hfactor=1
  a_tmp = 0.3;
// Highest depth ratio with hfactor=0
  b_tmp = 0.1;
...
#pragma omp parallel for private (dk,dk_inv,k)
    for (k = 0; ((long )k) <= D -> number_of_elements - 1; k += 1) {
...
#pragma omp parallel for private (k0,k1,k2,k3,k)
  for (k = 0; ((long )k) <= D -> number_of_elements - 1; k += 1) {
    k3 = k * 3;
    k0 = D -> surrogate_neighbours[k3];
    k1 = D -> surrogate_neighbours[k3 + 1];
    k2 = D -> surrogate_neighbours[k3 + 2];
    if ((D -> height_centroid_values[k0] < D -> minimum_allowed_height | k0 == k) & (D -> height_centroid_values[k1] < D -> minimum_allowed_height | k1 == k) & (D -> height_centroid_values[k2] < D -> minimum_allowed_height | k2 == k)) {
...
// Isolated wet cell -- constant stage/depth extrapolation
        ...
        D -> stage_edge_values[k3 + 1] = D -> stage_centroid_values[k];
        D -> stage_edge_values[k3 + 2] = D -> stage_centroid_values[k];
//max(stage_centroid_values[k]-bed_centroid_values[k],0.);
        dk = D -> height_centroid_values[k];
        D -> height_edge_values[k3] = dk;
        D -> height_edge_values[k3 + 1] = dk;
        D -> height_edge_values[k3 + 2] = dk;
...
// So hfactor = depth_ratio*(c_tmp) + d_tmp, but is clipped between 0 and 1.
      hfactor = fmax(0.,(fmin(c_tmp * fmax(hmin,0.0) / fmax(hc,1.0e-06) + d_tmp,(fmin(c_tmp * fmax(hc,0.) / fmax(hmax,1.0e-06) + d_tmp,1.0)))));
...
#pragma omp parallel for private (i) firstprivate (k3) num_threads(3)
        for (i = 0; i <= 2; i += 1) {
...
#pragma omp parallel for private (dk,i) num_threads(3)
      for (i = 0; i <= 2; i += 1) {
