...
// double _openmp_compute_fluxes_central(
// int K, int KI, int KI2, int KI3, int B, int RW, int RW5, int SubSteps,
// double D_edgelengths[KI],
// double D_normals[KI2],
// double D_edge_timestep[KI],
// double D_radii[K],
// double D_areas[K],
// long D_tri_full_flag[K],
// double D_stage_explicit_update[K],
// double D_xmom_explicit_update[K],
// double D_ymom_explicit_update[K],
// double D_stage_centroid_values[K],
// double D_height_centroid_values[K],
// double D_bed_centroid_values[K],
// double D_stage_edge_values[KI],
// double D_xmom_edge_values[KI],
// double D_ymom_edge_values[KI],
// double D_bed_edge_values[KI],
// double D_height_edge_values[KI],
// double D_edge_flux_work[KI3],
// double D_pressuregrad_work[KI],
// double D_max_speed[K],
// double D_stage_boundary_values[B],
// double D_xmom_boundary_values[B],
// double D_ymom_boundary_values[B],
// double D_boundary_flux_sum[SubSteps],
// long D_neighbours[KI],
// long D_neighbour_edges[KI],
// long D_edge_flux_type[KI],
// long D_edge_river_wall_counter[KI],
// long D_riverwall_rowIndex[RW],
// double D_riverwall_elevation[RW],
// double D_riverwall_hydraulic_properties[RW5],
// struct domain *D,
// double timestep)

double _openmp_compute_fluxes_central(struct domain *D,double timestep)
{
// Local variables
...
#pragma omp parallel for private(k, i, ki, ki2, ki3, n, m, nm, nm3, ii, max_speed_local, length, inv_area, zl, zr, h_left, h_right, z_half, ql0, ql1, ql2, qr0, qr1, qr2, edgeflux0, edgeflux1, edgeflux2, bedslope_work, normal_x, normal_y, hle, hre, zc, zc_n, Qfactor, s1, s2, h1, h2, pressure_flux, hc, hc_n, tmp, h_left_tmp, h_right_tmp, speed_max_last, weir_height, edge_timestep, RiverWall_count) reduction(min : local_timestep)
  for (k = 0; k <= K - 1; k += 1) {
// if (k > kmax) kmax=k;
// if (k < kmin) kmin=k;
    speed_max_last = 0.0;
...
// Get left hand side values from triangle k, edge i
      ql0 = D -> stage_edge_values[ki];
      ql1 = D -> xmom_edge_values[ki];
      ql2 = D -> ymom_edge_values[ki];
...
// Convert negative flag to boundary index
        m = -n - 1;
        qr0 = D -> stage_boundary_values[m];
        qr1 = D -> xmom_boundary_values[m];
        qr2 = D -> ymom_boundary_values[m];
...
// Linear index (triangle n, edge m)
        nm = n * 3 + m;
        nm3 = nm * 3;
        qr0 = D -> stage_edge_values[nm];
        qr1 = D -> xmom_edge_values[nm];
        qr2 = D -> ymom_edge_values[nm];
...
// Edge flux computation (triangle k, edge i)
      _openmp__flux_function_central(ql0,ql1,ql2,qr0,qr1,qr2,h_left,h_right,hle,hre,normal_x,normal_y,epsilon,z_half,limiting_threshold,g,&edgeflux0,&edgeflux1,&edgeflux2,&max_speed_local,&pressure_flux,hc,hc_n,low_froude);
...
// Weir flux adjustment
          _openmp__adjust_edgeflux_with_weir(&edgeflux0,&edgeflux1,&edgeflux2,h_left_tmp,h_right_tmp,g,weir_height,Qfactor,s1,s2,h1,h2,&max_speed_local);
        }
      }
// Multiply edgeflux by edgelength
      ...
      edgeflux1 *= length;
      edgeflux2 *= length;
      D -> edge_flux_work[ki3 + 0] = -edgeflux0;
      D -> edge_flux_work[ki3 + 1] = -edgeflux1;
      D -> edge_flux_work[ki3 + 2] = -edgeflux2;
...
// printf(" %d %d, %d local_timestep %f  %f\n", ID, k, i, local_timestep, D->edge_timestep[ki]);
// if (n >= 0) {
...
#pragma omp parallel for private(k, i, ki, ki2, ki3, n, inv_area)
  for (k = 0; k <= K - 1; k += 1) {
...
#pragma omp parallel for private (r,r0,i)
  for (i = 0; i <= 2; i += 1) {
    if (dqv[i] < -TINY) 
      r0 = qmin / dqv[i];
    if (dqv[i] > TINY) 
...
#pragma omp parallel for private (i) firstprivate (k3)
        for (i = 0; i <= 2; i += 1) {
...
#pragma omp parallel for private (dk,i)
      for (i = 0; i <= 2; i += 1) {
...
#pragma omp parallel for private (k)
  for (k = 0; k <= number_of_elements - 1; k += 1) {
...
#pragma omp parallel for private (dk,dk_inv,k) firstprivate (minimum_allowed_height)
    for (k = 0; k <= number_of_elements - 1; k += 1) {
...
// end of if (dk > minimum_allowed_height)
      }
// end of for
    }
// end of if (D->extrapolate_velocity_second_order==1)
  }
// If a triangle is surrounded by dry cells (or dry cells + boundary
// condition) set its momentum to zero too. This prevents 'pits' of
// of water being trapped and unable to lose momentum, which can occur in
...
#pragma omp parallel for private(k, k3, k0, k1, k2)
  
#pragma omp parallel for private (k0,k1,k2,k3,k)
  for (k = 0; k <= D -> number_of_elements - 1; k += 1) {
    k3 = k * 3;
    k0 = D -> surrogate_neighbours[k3];
    k1 = D -> surrogate_neighbours[k3 + 1];
    k2 = D -> surrogate_neighbours[k3 + 2];
...
#pragma omp parallel for private(k, k0, k1, k2, k3, k6, coord_index, i, dx1, dx2, dy1, dy2, dxv0, dxv1, dxv2, dyv0, dyv1, dyv2, dq0, dq1, dq2, area2, inv_area2, x, y, x0, y0, x1, y1, x2, y2, xv0, yv0, xv1, yv1, xv2, yv2, dqv, qmin, qmax, hmin, hmax, hc, h0, h1, h2, beta_tmp, hfactor, dk, dk_inv, a, b)
  for (k = 0; k <= number_of_elements - 1; k += 1) {
// Don't update the extrapolation if the flux will not be computed on the
// next timestep
// if(D->update_extrapolation[k]==0){
...
#pragma omp parallel for private(k, k3, i, dk)
  for (k = 0; k <= D -> number_of_elements - 1; k += 1) {
    if (D -> extrapolate_velocity_second_order == 1) {
// Convert velocity back to momenta at centroids
      D -> xmom_centroid_values[k] = D -> x_centroid_work[k];
