...
#         self.Ke = 6.7e-7
        self.Ke = num.ones_like(self.depth) * 6.7e-7
    ...
    def initialize_arrays(self):
        ...
        Quantity(self.domain, name='shear_stress', register=True)
        Quantity(self.domain, name='vel', register=True)
...
#         self.domain.set_quantity('ee', self.S, location='centroids')
    ...
    def calculate_u_slope(self):
        ...
        quant = self.domain.quantities['vel']
        quant.compute_gradients()
        
        du = num.sqrt((quant.x_gradient/ self.dx)**2 + (quant.y_gradient/ self.dx)**2)
    ...
    def update_quantities(self):
        ...
        if sum(self.ind) > 0: 
            ...
            self.du = self.calculate_u_slope()
...
#         self.Ke = 0.2e-6 / self.tau_crit**(0.5)
        ...
        shear_stress = (self.rho_w * self.u_star**2 +
                        self.rho_w * self.U * self.depth * self.du)
        
        self.domain.set_quantity('shear_stress', shear_stress, location='centroids')
        
        
        edot = self.Ke[self.ind] * (shear_stress[self.ind] - self.tau_crit)
        edot[edot<0.0] = 0.0
    ...
    def deposition(self):
        ...
        self.d_star[self.conc[self.ind] == 0] = 0
    ...
    def sediment_flux(self):
        ...
        for k in self.bdry_indices:
            for i in range(3):
                ...
                if (n < 0) & (self.depth[k] > 0.5):
                
                    sed_flux[k,i] =  edge_flux[k,i] * self.inflow_concentration
                    
                elif (n < 0) & (self.depth[k] > 0.3):
                
                    sed_flux[k,i] =  edge_flux[k,i] * 0.5*self.inflow_concentration
